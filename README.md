...
│  sounddevice real-time streams · sounddevice mixing engine      │
...
| **Python fallback** | `SfizzEnginePython` provides root-pitch-only playback when the C++ extension is absent |
...
| **Python fallback** | `DsEnginePython` provides root-pitch-only playback; no pitch shifting in the fallback path |
...
| Feature | Description |
|---|---|
| **Audio File Tracks** | Import WAV, MP3, FLAC, OGG clips (including OpenUTAU vocal exports) onto the timeline; each clip shows a waveform thumbnail |
...
Download a free General MIDI SoundFont (e.g. **GeneralUser GS** or **FluidR3 GM**) and place the `.sf2` file in the `soundfonts/` folder at the project root. The app discovers it automatically on startup and assigns each imported MIDI track its own GM instrument.

Recommended free SF2 files:
- **GeneralUser GS** — https://schristiancollins.com/generaluser.php (~30 MB, recommended)
- **FluidR3 GM** — `sudo apt install fluid-soundfont-gm` (Linux) or download manually
...
├── main.py                          # Entry point
├── requirements.txt
├── soundfonts/                      # Drop .sf2 files here (auto-discovered)
├── assets/
│   ├── icons/
│   └── screenshots/
...
└── core/                            # Python application layer
    ...
    │
    ├── # ── Audio tracks & FX ─────────────────────────────────────────────
    ├── audio_file_player.py         # sounddevice mixing engine for audio file tracks
...
| `pygame` | Gamepad input only (no audio use) |
| `mido` | MIDI file I/O |
| `python-rtmidi` | External MIDI device support |
| `numpy` | Audio buffer arithmetic |
| `scipy` | Signal processing utilities |
| `soundfile` | WAV / FLAC read-write |
| `sounddevice` | Real-time audio I/O — mixing engine for audio tracks, SFZ, DS, VST3 |
| `pedalboard` | VST3 / Audio Unit plugin hosting + audio file decoding + resampling |
| `librosa` | Audio analysis for AI tools |
| `scikit-learn` | ML utilities for AI tools |
| `sentence-transformers` | Text embedding for AI Generative Reverb IR search |
...
SBS-Synth Master runs on **macOS** and **Windows** (and Linux for development).

| Component | macOS | Windows |
|---|---|---|
| GUI (PySide6 / Qt6) | ✅ | ✅ |
| FluidSynth (SF2) | ✅ CoreAudio | ✅ WASAPI / DirectSound |
| sounddevice audio engine | ✅ CoreAudio | ✅ WASAPI shared |
| Audio file playback (WAV, MP3, FLAC, OGG) | ✅ | ✅ |
| Sample rate conversion (any → 44100 Hz) | ✅ via pedalboard | ✅ via pedalboard |
| OpenUTAU vocal WAV import | ✅ | ✅ |
| SFZ / Decent Sampler / VST3 | ✅ | ✅ |
| Audio Unit plugins | ✅ | ✗ (macOS only) |
| MP3 export | ✅ pedalboard or ffmpeg | ✅ pedalboard or ffmpeg |
| MIDI drag-and-drop with GM auto-assign | ✅ | ✅ |

**FluidSynth system library** must be installed separately:
- macOS: `brew install fluid-synth`
- Windows: download the installer from https://www.fluidsynth.org and add the `bin/` folder to PATH
